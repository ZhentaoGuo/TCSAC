...
from dataset import ThreeDLoader_TCSAC
from benchmark_utils import set_seed, icp_refine
from utils.timer import Timer
from TCSAC import Matcher_TCSAC
...
def eval_3DMatch_scene(loader, matcher, trans_evaluator, cls_evaluator, scene, scene_ind, config, use_icp):
    """
    Evaluate our model on 3DMatch testset [scene]  在3DMatch测试集[scene]上评估我们的模型
    """
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(num_pair)):
            ...
            #################################
            data_timer.tic()
            src_data, tgt_data, gt_trans = loader.get_data(i)
            data_time = data_timer.toc()

            #################################
            # 2. match descriptor and compute rigid transformation  匹配描述子并计算刚性变换
            #################################
            model_timer.tic()
            pred_trans, pred_labels, src_keypts_corr, tgt_keypts_corr = matcher.estimator(src_data, tgt_data)
            model_time = model_timer.toc()

            #################################
            # 3. generate the ground-truth classification result  生成真实分类结果
            #################################

            # if type(src_keypts_corr) is np.ndarray:
            #     print("变量是NumPy数组")
            # elif torch.is_tensor(src_keypts_corr):
            #     print("变量是张量")
            gt_trans=torch.tensor(gt_trans)
            src_keypts_corr=torch.tensor(src_keypts_corr)
            tgt_keypts_corr=torch.tensor(tgt_keypts_corr)            
            gt_trans= gt_trans.to('cuda:0')
            src_keypts_corr=src_keypts_corr.to('cuda:0')
            gt_trans=gt_trans.double()
            src_keypts_corr=src_keypts_corr.double()
...
def eval_3DMatch(config, use_icp):
    """
    Collect the evaluation results on each scene of 3DMatch testset, write the result to a .log file.  收集3DMatch测试集中每个场景的测试结果，将结果写入一个log文件。
    """
    scene_list = [
        '7-scenes-redkitchen',
    ...
    for scene_ind, scene in enumerate(scene_list):
        loader = ThreeDLoader_TCSAC(root=config.data_path,
        ...
        matcher = Matcher_TCSAC(inlier_threshold=config.inlier_threshold,
                 num_node=config.num_node,
                 use_mutual=config.use_mutual,
    ...
    for scene, stats in all_stats.items():
        correct_pair = np.where(stats[:, 0] == 0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', default='config_json/TCSAC_config_3DMatch.json', type=str, help='snapshot dir')
    parser.add_argument('--solver', default='RANSAC', type=str, choices=['SVD', 'RANSAC'])
    ...
    if not os.path.exists("./logs"):
        os.makedirs("./logs")
    log_filename = f'logs/TCSAC_config_3DMatch-{config.descriptor}.log'
