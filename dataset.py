...
class ThreeDLoader(data.Dataset):
    ...
    def get_data(self, index):
        ...
        if self.descriptor == 'fcgf':
            ...
            src_features = src_data['features']
            tgt_features = tgt_data['features']
        ...
        gt_trans = np.linalg.inv(self.gt_trans[key])  # the given ground truth trans is target-> source    ###目标到源
...
class ThreeDLoader_TCSAC(data.Dataset):
    def __init__(self,
                 root,
                 descriptor='fpfh',
                 inlier_threshold=0.10,
    ...
    def get_data(self, index):
        ...
        # load point coordinates and pre-computed per-point local descriptors
        if self.descriptor == 'fcgf':
            src_data = o3d.io.read_point_cloud(f"{self.root}/fragments/{scene}/cloud_bin_{src_id}.ply")
            tgt_data = o3d.io.read_point_cloud(f"{self.root}/fragments/{scene}/cloud_bin_{tgt_id}.ply")
        elif self.descriptor == 'fpfh':
            src_data = o3d.io.read_point_cloud(f"{self.root}/fragments/{scene}/cloud_bin_{src_id}.ply")
            tgt_data = o3d.io.read_point_cloud(f"{self.root}/fragments/{scene}/cloud_bin_{tgt_id}.ply")
...
class WorkpieceLoader(data.Dataset):
    def __init__(self,
                 root,
                 descriptor='fpfh',
                 inlier_threshold=0.10,
    ...
    def get_data(self, index):
        ...
        # compute ground truth transformation
        gt_trans = self.gt_trans[key]  # the given ground truth trans is target-> source    ###目标到源
...
class WorkpieceLoader_sc2(data.Dataset):
    def __init__(self,
                 root,
                 descriptor='fpfh',
                 inlier_threshold=0.10,
